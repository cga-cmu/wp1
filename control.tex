...
A later white paper will address robustness to modeling error and unmodeled
...
\subsubsection{What degress of freedom are controlled?}
...
or other coordinates. We refer to these virtual degress of freedom as {\bf synergies.}
...
\subsubsection{How is a degree of freedom controlled?}

It is useful for human programmers
to provide them with idealized abstractions of control of a single degree of
freedom. Here are some common alternatives:
\begin{itemize}
\item
...
PID or more complex feedback control laws are implemented at this level.
...
State estimation is done at this level to compensate for sensor noise.
...
\begin{equation}
\vtau = \vtau_{\invdynw} + \mJ_o^{\tr} {\vf_o}_d
...
and any addtional constraint equations for ground contacts (feet on the ground):
\begin{equation}
...
quality IMU at the pelvis, multiple low quality (but cheap) MEMs IMUs
spread throughout the robot. We also take into account feet and hand
...
We use the robot joint sensing (kinematics) to create a virtual sensor that measures
...
external force exerted on the center of mass of the robot,
...
unknown locations on the robot, which is quite likely when
...
allowed, our early warning system
...
support warns the controller that the robot might fall if the external
force is maintained.
...
We assume the external force is due to the robot's actions, and not
due to external disturbances such as wind, a moving support platform
(although earthquakes do happen at the DRC location in California),
or external agents pushing on the robot.
The current
behavior is stopped and the robot ``frozen'' in place.
...
predicted, there are several things the humanoid can do, such
...
the robot corrects the next foot placement and timing
using a simple heuristic to avoid self collision.
We would only step left if the robot is falling left, and similarly
...
Examples of typically unmodeled dynamics (non-parametric modeling error) include:
...
loose or slack operator tissue and strap.
...
So feedforward control is robust, and we don't have to worry about it. We do have
...
make this control work reliably. The fact that this has been done on many actual
