...
% review? \url{http://ieeexplore.ieee.org/xpl/login.jsp?tp=&arnumber=6907051}
...
Often this type of control is referred to as a ``torque control inner loop''
with a ``position/velocity control outer loop''. In this approach
the inner and outer loops are combined in one equation (eq.~\ref{eq:valve}),
and there is no
longer ``inner'' and ``outer'' control quantities. All feedback is
...
\subsection{``Sentence'' Control: Behavior Selection}
...
%~\cite{certainty-eq,separation-prin}.
...
map operator ``muscle commands'' $\vu_{\myo}$ to operator joint torques $\vtau_{\myo}$.
\begin{equation}
...
%\cite{Stephens-thesis,Whitman10,Whitman-thesis}. 
Unlike those
% ~\cite{mistry10,usc_id1}
...
described in.
\url{http://www.cs.cmu.edu/~cga/drc}.
%~\cite{xinji_com_est,Xinjilefu-thesis}.
...
locomotion for an exoskeleton robot.
\begin{verbatim}
...
smoothing method is introduced and a pre-transition
