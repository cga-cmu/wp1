...
the subscript $\ox$ in dynamics equations.
We can also separate out the operator dynamics (indicated by
the subscript $\myo$) from the exoskeleton dynamics (indicated by
the subscript $\myx$) if this is more convenient
(we separate them out and ignore them in what immediately follows).
...
One could add
\begin{equation}
\vtau = \vtau_{\xinvdyn} + \mJ_w^{\tr}(\vq) {\vf_w}_d
\end{equation}
...
\begin{equation}
\vtau = \vtau_{\xinvdyn} + \mJ_w^{\tr}(\vq) {\vf_w}_d + \mJ_w^{\tr}(\vq) \mK_{\vf_w} (\vf_w - {\vf_w}_d)
\end{equation}
...
\begin{equation}
\vtau = \vtau_{\xinvdyn} - \mK_{\vq} ( \hat{\vq} - \vq_d ) - \mK_{\dot{\vq}} \hat{\dot{\vq}}
...
+ \mJ_w^{\tr}(\vq) \vf_w = \mS \vu + \mJ_{\ox}^{\tr}(\vq) \vf_{\ox}
\label{eq:dynamics2}
\end{equation}
