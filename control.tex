...
We implemented two kinds of Kalman filters for state estimation.
Low level state estimation is done by an extended Kalman filter that takes into
account joint position, actuator load measurements, actuator commands,
and the dynamic model of the robot,
and measured contact forces (feet and wrists)~\cite{}.
...
implemented on our Atlas humanoid robot. During the DARPA
Robotics Challenge Finals, it successfully saved our robot from
