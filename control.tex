...
2) We recommend the use of dynamic models of the exoskeleton, as has been done
for the last decade starting with the BLEEX exoskeleton.

3) We recommend using online optimization as part of the Lowest Level Controller
in the form of Quadratic Programming. This was tested by the top biped robot
teams in the DARPA Robotics Challenge, and worked well.

4) We recommend applying as many sensors as possible, and asking the performers
...
the exoskeleton. We would like to see high quality direct velocity sensing
such as high count (100,000 counts/revolution) rotary encoders and 
analog linear tachometers.

5) {\bf Controversial:} We recommend a ``symbiotic'' control system design,
which expects the operator to adapt to the exoskeleton and its control,
and which has the control customized for the operator.

6) {\bf Controversial:} We recommend making use of virtual model control specialize
